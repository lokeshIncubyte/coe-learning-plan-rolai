...
Audits one or more `SKILL.md` files against eight dimensions distilled from Anthropic's skill-authoring guidance and the `anthropics/skills` reference repo. Produces a written report with per-skill scorecards, cross-cutting issues, top fixes, and a "what's well-designed" affirmation section.
...
Eight dimensions ordered by leverage. Frontmatter and abstraction-fit compound across every invocation; verbosity issues compound less.
...
Severity: dead refs = **high**; fossilized paths, methodology bleed, off-workflow references, or project-specific vocabulary = **medium**; menus or magic constants = **low**.
...
Severity: high filler ratio = **medium**; no imperative voice = **low**; distractors near critical instructions = **high**.
...
- [ ] 4. **Multi-skill mode only — synthesize cross-cutting patterns.** Issues recurring across ≥3 skills go into the cross-cutting section. Three is the threshold because two-skill repeats are coincidence; three-skill repeats are patterns the project will keep reproducing. Common ones: fossilized paths, inconsistent HITL markers, missing `## Inputs` sections, orchestrator leakage.
...
- **Dimensions are ordered by leverage**, not by check order. Frontmatter (1) and abstraction-fit (7) compound across every invocation; verbosity (8) compounds less.
- **Don't grade what's merely stylistic.** "Section order is unusual but consistent with the rest of the project" is fine. Style coherence within the project matters more than universal style.
- **Source for criteria:** Anthropic skill-authoring best practices and the `anthropics/skills` reference repo. When global guidance and project conventions conflict, defer to project conventions and note the divergence.
- **The auditor is itself a bullseye-fit skill** — one input shape (path), one output artifact (report), one HITL contract (review at the end). Eat your own dogfood.
